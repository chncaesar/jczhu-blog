---
title: I Don't Understand Embedded. I Was Still the Only One Who Could Ship It.

description: My agent passed every test it could run. The tests it could run lived in the wrong universe — and closing that gap turned out to be the only work I could actually do.
...
**Draw a map for the next one.** I wrote the hard-won facts down as rules: the manifest format, the missing setting, a table of which pin names differ. This froze the intelligence the seven shots cost me. But it's the most fragile of the three: the next agent has no memory of this session, and nothing guarantees it will load those rules before it starts guessing again. A map only helps if the scout reads it. I wrote the rules for an executor that may never open them.
